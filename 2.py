...
class SpeakerGLWidget(QGLWidget):
    ...
    def draw_drivers(self):
        ...
        # Adjust woofer size when all three drivers are selected
        if all(self.drivers.values()):
            driver_sizes["woofer"] = 0.6  # Make woofer smaller when all three are selected
        
        elif "woofer" in active_drivers and "subwoofer" in active_drivers:
            # Only woofer and subwoofer are selected - make subwoofer bigger
            driver_sizes["subwoofer"] = 1.2

        # Adjust positions based on active drivers
